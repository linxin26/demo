...
public class MyProxyImpl {
	...
	/**
	 * 暴露给用户的动态代理接口，返回某个接口的动态代理对象，注意本代理实现需和com.cuishen.myAop.
	 * InterceptorHandler拦截器配合
	 * 使用，即用户要使用本动态代理，需先实现com.cuishen.myAop.InterceptorHandler拦截器接口 <br>
	 * 使用方法如下: <br>
	 * <code>
	 ...
	 *            String 要动态代理的接口类名, e.g test.StudentInfoService
	 * @param classToProxy
	 *            String 要动态代理的接口的实现类的类名, e.g test.StudentInfoServiceImpl
	 ...
	 */
	public static Object newProxyInstance(String interfaceClassName,
			String classToProxy, String interceptorHandlerImplClassName)
			throws InstantiationException, IllegalAccessException,
			NotFoundException, CannotCompileException, ClassNotFoundException {
		Class<?> interfaceClass = Class.forName(interfaceClassName);
		Class<?> interceptorHandlerImplClass = Class
				.forName(interceptorHandlerImplClassName);
		return dynamicImplementsInterface(classToProxy, interfaceClass,
				interceptorHandlerImplClass);
	}
	 ...
	 *            Class 要动态代理的接口类, e.g test.StudentInfoService
	 * @param interceptorHandlerImplClass
	 *            Class 用户提供的拦截器接口的实现类
	 * @return Object 返回某个接口的动态代理对象
	 * @throws NotFoundException
	 * @throws CannotCompileException
	 * @throws InstantiationException
	 * @throws IllegalAccessException
	 */
	private static Object dynamicImplementsInterface(String classToProxy,
			Class<?> interfaceClass, Class<?> interceptorHandlerImplClass)
			throws NotFoundException, CannotCompileException,
			InstantiationException, IllegalAccessException {
		ClassPool cp = ClassPool.getDefault();
		String interfaceName = interfaceClass.getName();
		// 动态指定代理类的类名
		String proxyClassName = interfaceName + PROXY_CLASS_NAME_SUFFIX
				+ proxyClassIndex++;
		// 要实现的接口的包名+接口名
		...
		for (int i = 0; i < methods.length; i++) {
			...
			dynamicImplementsMethodsFromInterface(classToProxy, cc, method,
					interceptorHandlerImplClass, i);
		}
	 ...
	 * @throws CannotCompileException
	 */
	private static void dynamicImplementsMethodsFromInterface(
			String classToProxy, CtClass implementer, Method methodToImpl,
			Class<?> interceptorClass, int methodIndex)
			throws CannotCompileException {
		String methodCode = generateMethodCode(classToProxy, methodToImpl,
				interceptorClass, methodIndex);
	 ...
	 */
	private static String generateMethodCode(String classToProxy,
			Method methodToImpl, Class<?> interceptorClass, int methodIndex) {
		...
		// 方法体
		methodDeclare.append(INTERCEPTOR_HANDLER_INTERFACE)
				.append(" interceptor = new ").append(interceptorImplName)
				.append("();\n");
		// 反射调用用户的拦截器接口
		methodDeclare
				.append("Object returnObj = interceptor.invoke(Class.forName(\""
						+ classToProxy
		...
		for (int i = 0; i < parameters.length; i++) {
			// ($w) converts from a primitive type to the corresponding wrapper
			// type: e.g.
			// Integer i = ($w)5;
			if (i != parameters.length - 1)
		...
		if (methodToImpl.getReturnType().isPrimitive()) {
			if (methodToImpl.getReturnType().equals(Boolean.TYPE))
				...
				methodDeclare
						.append("return ((Integer)returnObj).intValue();\n");
				...
				methodDeclare
						.append("return ((Float)returnObj).floatValue();\n");
				...
				methodDeclare
						.append("return ((Short)returnObj).shortValue();\n");
		} else {
			methodDeclare.append("return (" + methodReturnType
					+ ")returnObj;\n");
		}
		methodDeclare.append("}");
